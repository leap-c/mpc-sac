...
def create_cfg(env: str, controller: str, seed: int) -> RunSacZopConfig:
    ...
    cfg.trainer.train_steps = 0  # No training - just run one step to trigger validation
    cfg.trainer.train_start = 0
    cfg.trainer.val_freq = 1  # Validate immediately
    cfg.trainer.val_num_rollouts = 1  # Run only 1 rollout
    cfg.trainer.val_deterministic = True
    cfg.trainer.val_num_render_rollouts = 1  # Render that 1 rollout
...
def run_sac_zop(
    ...
    state_dict = torch.load("best_pi.ckpt", map_location="cpu", weights_only=False)
    trainer.pi.load_state_dict(state_dict)
